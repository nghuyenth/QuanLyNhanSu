...
namespace Nhom3_QuanLyNhanSu
{
    public partial class FrmNhanVien : Form
    {
        ...
        private void EnableTextBox1(bool b)
        {
            foreach (Control ct in groupBox4.Controls)
            {
                if (ct is ComboBox)
        ...
        private void ShowDetail(DataGridViewRow row)
        {
            if (row != null && (tab != null && (tab.action == ActionForm.KHONG || tab.action == ActionForm.SUA)) || tab == null || BeforeInsert)
            {
                ...
                string CVPM = row.Cells[12].Value.ToString();
                rdCBQL.Checked = CVPM.Equals("2");
                rdCVGV.Checked = CVPM.Equals("3");
        ...
        private void EnableButton(bool b) {
            ...
            btnXemCTCMND.Enabled = isAdmin == 1 || isAdmin == 2;
            btnResetPass.Enabled = isAdmin == 1 || isAdmin == 2;
            btnThemTN.Enabled = isAdmin == 1 || isAdmin == 2 ;
            btnSuaTN.Enabled = isAdmin == 1 || isAdmin == 2;
            btnXoaTN.Enabled = isAdmin == 1 || isAdmin == 2;
        }
        ...
        private void Sua()
        {
            ...
            tab.action = ActionForm.SUA;
                ...
                else if (isAdmin == 2)
                {
                    ...
                    if (rdCBQL.Checked) 
                    {
                        MessageBox.Show("Bạn không thể sửa Cán Bộ Quản lí. Hãy liên hệ Admin để thay đổi thông tin");
                        changeStateButton(true, true, false, true, true);
        ...
        private void Xoa()
        {
            ...
            {
                if (rdCBQL.Checked) {
                    MessageBox.Show("Bạn không thể xóa Cán bộ Quản lí.");
                    changeStateButton(true, true, false, true, true);
                }
                else
                {
        ...
        public FrmNhanVien(int isAdmin)
        {
            InitializeComponent();
        ...
        {
            if(isAdmin==1 && dataGridView1.CurrentRow!=null || isAdmin == 2 && dataGridView1.CurrentRow != null)
            {
                if (!checkNgaySinh())
                {
